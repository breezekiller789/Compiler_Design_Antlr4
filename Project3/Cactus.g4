...
statement [int reg, int label] returns [int nreg, int nlabel] :
        ...
        | {System.out.println("\t#if");} IF LeftParen m = bool_expression[$reg, $label+3, $label, $label+1] {System.out.println("L" + $label + ":\t#then");} RightParen LeftBrace y = statements[$m.nreg, $m.nlabel] {System.out.printf("\tb\tL%d\n", $label+2);System.out.printf("L%d:\t#else\n", $label+1);} RightBrace n = else_statement[$y.nreg, $y.nlabel] {System.out.printf("L%d:\t#end if\n", $label+2);$nreg = $m.nreg;$nlabel = $m.nlabel;}
        | {System.out.printf("L%d:\t#while\n", $label);} WHILE LeftParen i = bool_expression[$reg, $label+3, $label+1, $label+2] {System.out.printf("L%d:\t#while body\n", $label+1);} RightParen LeftBrace z = statements[$i.nreg, $i.nlabel] {System.out.printf("\tb\tL%d\n", $label);System.out.printf("L%d:\t#end while\n", $label+2);$nreg = $z.nreg; $nlabel = $z.nlabel;} RightBrace
...
bool_expression1 [int reg, int label, int True, int False] returns [int nreg, int nlabel] :
        OR {System.out.printf("%d\n\tb\tL%d\n", $True, $label);System.out.printf("L%d:\n", $label);} x = bool_term[$reg, $label+1, $True, $False] y = bool_expression1[$x.nreg, $x.nlabel, $True, $False] {$nreg=$x.nreg;$nlabel=$x.nlabel;}
...
rel_expression [int reg, int label, int True, int False] returns [int nreg, int nlabel] :
        ...
        x = arith_expression[$reg] z = relation_op y = arith_expression[$x.nreg] {System.out.printf("\t%s\t%c%c%d, %c%c%d, %c", $z.s, '$', 't', $x.place, '$', 't', $y.place, 'L');$nreg = $y.nreg-2;$nlabel = $label;}
        ;
